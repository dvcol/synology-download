...
import {TasksState} from "../slices/tasks.slice";
...
export const getFilteredTasks = createSelector(
    getTasks,
    getStatuses,
    (tasks, statuses) => statuses?.length ? tasks.filter(t => statuses.includes(t.status)) : tasks
)

export const getCountByStatus = createSelector(
    getTasks,
    (tasks) => {
